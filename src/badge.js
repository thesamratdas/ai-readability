...
export function makeBadge(grade, score) {
  ...
  // Label "AI-Ready" (~8 chars) + value ("A"-"F": 1 char, fits 28px; "N/A": needs more room)
  const lw = 68, vw = grade.length > 1 ? grade.length * 6 + 20 : 28, w = lw + vw;
  const lx = Math.floor(lw / 2);
  const vx = lw + Math.floor(vw / 2);
  return [
    `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="20" role="img" aria-label="AI-Ready: ${grade}">`,
    `  <title>AI-Ready: ${grade} (score ${score}/100)</title>`,
    ...
    `    <text x="${lx}" y="15" fill="#010101" fill-opacity=".3">AI-Ready</text>`,
    `    <text x="${lx}" y="14">AI-Ready</text>`,
